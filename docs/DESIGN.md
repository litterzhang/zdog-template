...
回溯天然有指数风险：多个相邻选择点，每个都有多个候选，而失败只在最后才暴露。

```
模板  ${a}.${b}.${c}.${d}.${e}.${f}.${re|expr=\d+}
输入  x.x.x.…x.nodigit          （N 个 '.'，结尾不是数字）
路径数 ≈ C(N, 6)
```

实测（无记忆化）：10 个点 74 µs、14 个点 281 µs、18 个点 1.4 ms、22 个点 **5.3 ms** ——
每多 4 个点约 4 倍。51 字节的输入就能拖垮一整批。

**记忆化只记失败，不记成功。** 理由是：从 `(opIdx, pos)` 出发能否成功，只取决于
`opIdx` 之后的算子与 `src[pos:]`，与"怎么走到这里的"无关（后续算子只写 `Spans`，
从不读）。而记成功需要连同写入的 span 一起复现，得不偿失。

判定"确实无解"的条件有两个，缺一不可：子树被**完整**探索过（不是被 visit 叫停的），
且期间 `visit` 一次都没被调用。

**门槛是自适应的**：记忆化表本身要分配，而绝大多数搜索几步就结束（尤其是批量日志里
"这行压根不匹配"）。所以先裸跑，`steps` 越过 64 才建表 —— 常见路径一分钱不花。
...
**输入翻倍、耗时约 4 倍 —— 二次级**。同样的 5 ms 现在能吃下 14 倍长的输入。
...
既然 `parse | format ≡ transform`，两条路的输入输出完全相同 —— 这是同一个
选择的两个选项，可以直接比：

| | ns/行 | 分配/行 |
|---|---|---|
| `transform` 一步到位 | **62.9** | **0** |
| `parse \| format` 两步 | **1842.4** | ~18 |
| ├ `parse` 这一半 | 266.3 | |
| └ `format` 这一半 | 1542.4 | |

**差距的根源是中间态的形态，不是任何一段代码的效率。**

```
transform      text ──[spans]──> text
               中间态是 (int32, int32) 对，指向原始 buffer，从不物化

parse | format text ──[NDJSON 文本]──> text
               中间态是实实在在的字节：必须写出来，再读回去
```

一旦中间态变成**文本**，就丢掉了"这段字节在哪"，只剩"这段字节是什么"。于是：

- **parse 侧**：把 span 指向的内容真的拷出来、加引号、转义 → 产生字节
- **format 侧**：重新扫描这些字节、识别边界、分配、装箱 → 重建值

这一去一回就是那 29 倍。实测里 `format` 的 1542 ns 中约 1356 ns 是纯
`json.Decode`（17 次分配/行），只有 ~200 ns 属于"搬值 + 渲染"。

> **这是可观察性的代价，不是实现缺陷。**
> `transform` 快，是因为它的中间态**从不离开进程边界**，可以一直是偏移量。
> 一旦你要把中间态拿出来看 —— 落盘、进消息队列、管道喂给 `jq` —— 它就必须
> 物化成字节。物化 + 重新解析，正是这 29 倍。

而且这个代价**不是 JSON 特有的**：任何文本中间态都要序列化再反序列化，
JSON 只是加剧了它（要转义、要装箱、要哈希）。真想更便宜，中间态得改成
基于 span 的二进制格式（原始行 + 偏移表），但那样就不能 `jq` 了 ——
**中间态的可读性与性能直接对立。**

### 所以怎么选

- **要吞吐** → `transform`。中间态不出边界，62.9 ns/行、零分配。
- **要在中间插一脚**（落盘审计、进队列、在别处加工）→ `parse | format`。
  付 29 倍换可观察性。
- **数据本来就不是文本**（数据库/API 取数）→ 只给 `target` 的纯渲染模式，
  不存在"绕一趟"的问题，1542 ns/行 就是它的固有成本。
...
| 限制 | 影响 | 规避 |
|---|---|---|
| ~~回溯无记忆化~~ | ✅ **已修**：失败记忆化把病态模板从指数级拉回二次级。6 洞模板 + 22 个定界符：5.3 ms → 56.7 µs（93 倍）；输入翻倍耗时约 4 倍而非指数 | — |
...
| **表达式路径每个被引用字段 1 次分配** | `any` 装箱的固有代价。实测边际成本：纯取值表达式 ~57 ns/个，函数调用 ~150 ns/个；快路径 78 ns/行、零分配 | 能用裸字段名就别写表达式。**函数调用的参数切片已改成复用求值栈**（4→3 次分配）。剩下的要消掉得换成带类型标签的 Value 结构体 —— 要动每个 AST 节点、每个函数、`Serialize`、`codec`，风险大而收益中等，**刻意不做** |
...
流式（`transform_stream` / `parse_stream` / `format_stream` / `verify_stream`）
的内存峰值 ≈ `chunk_size` + 最长一行，**与输入总大小无关**。
唯一的讲究是**块边界必须落在行边界上** —— 把一行切成两半，两半都不匹配模板。
分块之后诊断里的行号由 SDK 换算成全局行号（依赖 Go 侧 `line N:` 的前缀格式，
是刻意的耦合，换来不必为行号再往 ABI 里塞参数）。


| 限制 | 影响 | 规避 |
|---|---|---|
| ~~CLI 全量读入内存~~ | ✅ **已修**：SDK 增加 `*_stream` 系列，CLI 全部改走流式。实测 122 MB 输入：**全量 +478 MB RSS → 流式 +0 MB** | — |
| **SDK 输出缓冲只增不减** | 处理过一次大输入后，`Template` 实例一直占着那块缓冲 | 大批量用完 `close()` 重建 |
| **重复块只能同名/重命名映射** | 组不支持表达式，只有 `{"目标块名": "源块名"}`。缺的是语义设计（`reverse(items)` 到底该倒序迭代还是倒序渲染？），不是编码 | 块内字段可以用表达式；块本身的重排需在模板层做。错误信息已改成明说"组不支持表达式" |
