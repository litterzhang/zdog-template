...
### 为什么 `transform` 比 `format` 快一个数量级

不是因为 `transform` 被优化过，而是**两者的工作量根本不同**。实测拆开
（`core/pipeline/compare_bench_test.go`）：

| | ns/行 | 分配/行 |
|---|---|---|
| `transform` 全程 | **59.8** | **0** |
| ├ 其中 parse 那一半 | 25.5 | 0 |
| **单纯 `json.Decode`，别的什么都不干** | **1356** | 17 |
| `format` 全程 | 1579 | 18 |

**光是 JSON 解码就是整个 `transform` 的 23 倍。** `format` 的 1579 ns 里只有
223 ns 属于"搬值 + 渲染" —— 把模板部分优化到 0，它仍然要 1356 ns。
**所以这个差距与模板引擎无关，是 JSON 作为输入格式的固有代价。**

根子上的原因：**`transform` 全程没有产生过一个"值"。**

```
源文   [T1] error
        ↑1  ↑5
parse   只记两对 int32：(1,3) (5,10)
fill    d.Values[i] = src[1:3]      ← 切片，共享同一块内存，零拷贝
Format  append(dst, ...)            ← 唯一一次拷贝
```

`"T1"` 从头到尾就是源文里那两个字节，从没被复制、解析或装箱。而 `format`
必须先把 JSON 变成通用数据结构：逐字符扫描、处理转义、为每个字符串分配新
内存、装箱成 `any`、塞进 map；用的时候再哈希查找取回来、类型 switch、
序列化进 arena。

> 一句话：**`transform` 的结构信息来自模板（编译期就固定了），
> `format` 的结构信息藏在输入数据里（每行都要重新发现一遍）。**
