...
- 重复块**内部**的迭代切分不会因块之后的算子失败而重试（块的终止位置本身已参与回溯）。
- 回溯没有记忆化；病态模板（多个相邻选择点 + 长输入）理论上可指数级。
  实际模板极少触发，需要时再加 packrat。
- 表达式路径每行约 3 次分配来自 `any` 装箱，改用带类型标签的 Value 结构体可消除。
...
| 门禁 | 阈值 | 实测 |
|---|---|---|
| T0 层 parse | ≤ 100 ns/行 | **49.85 ns/行**（1444 MB/s，0 分配） |
| T2 岛 parse | — | 90.42 ns/行（0 分配） |
| T3 重复块 | — | 69.38 ns/迭代（0 分配） |
| 端到端 transform（快路径） | ≥ 10 M行/秒 | **10.12 M行/秒**（98.81 ns/行，0 分配） |
| 端到端 transform（表达式） | — | 4.06 M行/秒（246.3 ns/行） |
| Python SDK 端到端 | ≥ 10 M行/秒 | **10.62 M行/秒**（94.14 ns/行） |
| SDK 相对纯 Python | ≥ 5x | **10.1x** |

> **基准方法**：ns 级基准必须给足迭代数（`-benchtime=1000000x`），
> 200x 主要测的是预热噪声。Python 侧取 5 轮最小值 —— 噪声只会**增加**耗时，
> 因此最小值是对真实成本最稳的估计；单次计时在共享机器上能差 20%
> （同一份代码曾测出 9.26 与 11.09 两个结果）。
