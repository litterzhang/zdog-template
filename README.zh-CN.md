...
```bash
uv tool install zdog-template-cli   # 或 pip install zdog-template-cli
ztpl demo                           # 自带的完整例子，不需要任何输入
```
...
wheel 里自带了原生库，不需要编译。目前构建了 `manylinux_2_28_x86_64` 与
macOS arm64；其它平台见[从源码构建](#从源码构建)。
